...
var weui = {
	alert : function(msg, title, callback) {		
		...
		alertHtml += '      <a href="javascript:;" class="weui_btn_dialog default">确定</a>';
		alertHtml += '  </div>';
		alertHtml += ' </div>';
		alertHtml += '</div>';
	...
	alertes : function(msg, title, callback) {		
		title = title ? title : "提示信息";
		var alertHtml = '<div class="weui_dialog_alert" style="display: none;">';
		alertHtml += '<div class="weui_mask"></div>';
		alertHtml += '<div class="weui_dialog">';
		...
		alertHtml += '    <div class="weui_dialog_bd"></div>';
		alertHtml += '    <div class="weui_dialog_ft">';
		alertHtml += '      <a href="javascript:;" class="weui_btn_dialog default">我知道了</a>';
		alertHtml += '  </div>';
		alertHtml += ' </div>';
		alertHtml += '</div>';
		if ($(".weui_dialog_alert").length > 0) {
	...
	alerts : function(msg, title, callback) {
		title = title ? title : "提示信息";
		var alertHtml = '<div class="weui_dialog_alerts" style="display: none;">';
		alertHtml += '<div class="weui_mask"></div>';
		alertHtml += '<div class="weui_dialog">';
		alertHtml += '    <div class="weui_dialog_hd"><strong class="weui_dialog_title">'
				+ title + '</strong></div>';
		...
		var weui_alerts = $(".weui_dialog_alerts");
		weui_alerts.show();
		weui_alerts.find(".weui_dialog_bd").html(msg);
		weui_alerts.find('.weui_btn_dialog').off("click").on('click',
				function() {
					weui_alerts.hide();
					if (callback) {
						callback();
	...
	confirm : function(msg, title,sure,fal, callback) {
		...
		var confirmHtml = '<div class="weui_dialog_confirm">'
				...
				+ '<a href="javascript:;" class="weui_btn_dialog default"></a>'
		...
		weui_confirm.find('.primary').text(
				sure = (sure ? sure : "确定"));
		weui_confirm.find('.default').text(
				fal = (fal ? fal : "取消"));
	...
	Loading : {
		...
		msg : function(msg, time, cls, icon) {
			var successHtml = '<div class="' + cls
					+ '" style="display: none;">'
					+ '<div class="weui_masks"></div>'
					+ '<div class="weui_toast" style="left:217px;">' + '<i class="' + icon
					+ '"></i>' + '<p class="weui_toast_content"></p>'
					+ '</div>' + '</div>';
			if ($("." + cls).length < 1) {
				$("body").append($(successHtml));
				$(".weui_masks").off("click").on("click",function() {
					$("." + cls).hide();
				})
			}
			var t = 3000;
